...
if ($role === 'jobseeker') {
    ...
    // Save file first so we can hash it
    ...
    // Duplicate checks — email, full name, ID hash (like signup.php)
    ...
    //        id_file_path(s) id_image_hash(s) password(s)  = 13s + i + 6s = 20
...
// ── EMPLOYER ─────────────────────────────────────────────────────────────────
    ...
    if (isset($_FILES['business_permit']) && $_FILES['business_permit']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['business_permit']['tmp_name'], $dest_path)) {
            ...
            // Log upload failure for employer
            $upload_error = error_get_last();
            $error_msg = $upload_error ? $upload_error['message'] : 'Unknown error';
